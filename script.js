...
const originalHeroes = [
  ...
  { name: "Ravi-Riina", role: "mehaanik", hp: 80, power: 10 },
  { name: "Sära-Siim", role: "mehaanik", hp: 75, power: 12 },
...
function renderHeroes(list) {
  ...
  list.forEach((hero) => {
    ...
    powerSpan.textContent = "Jõud: " + hero.power;
...
//  - renderHeroes(heroes);
//  - appendLog("Mehaanikud parandasid tanke (+15 HP)");
function healTanks() {
  let healedCount = 0;
  heroes.forEach((hero) => {
    if (hero.role === "tank") {
  ...
  renderHeroes(heroes);
  appendLog("Mehaanikud parandasid tanke (+15 HP) - tanke parandatud: " + healedCount);
}
